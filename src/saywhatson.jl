# src/saywhatson.jl — SayWhatsOn, Meilenstein 3 (Bildschirm + Sprache + Editor + Systemkontext + Fensterliste + Zwischenablage)
#
# 📍 ORT IN DER PROJEKTARCHITEKTUR:
#   Eingabe:       Strg+Druck → Screenshot + Systemkontext + Fensterliste;
#                  Strg+Umschalt+Druck → Bild aus der Zwischenablage (Foto/Dokument/Scan);
#                  Umschalt+Druck → letzte Beschreibung im Editor lesen
#   Verarbeitung:  Bild (+ Kontext) → austauschbares Vision-LLM-Backend → Text
#   Ausgabe:       SAPI-Sprachausgabe + Konsole (zentral: `ausgeben`); optional Editor (JAWS)
#   Verantwortlichkeiten:
#     - Drei globale Hotkeys registrieren UND konsumieren (RegisterHotKey)
#     - Bildschirm ODER Zwischenablagebild aufnehmen; bei Bildschirm zusaetzlich
#       Systemstatus und Fensterliste sammeln; beschreiben lassen; sprechen
...
#   Strg+Umschalt+Druck -> Bild aus der Zwischenablage beschreiben (Foto, Dokument, Scan)
...
const HOTKEY_CLIPBOARD::Cint    = 3          # Strg+Umschalt+Druck
...
const PROMPT_BILD::String = """
Du beschreibst ein Bild fuer einen blinden Menschen. Das Bild kann ein Foto, ein gescanntes Dokument, ein Brief, eine Verpackung, ein Etikett, ein Ausschnitt einer Webseite oder Ähnliches sein. Es ist KEIN Bildschirm-Orientierungsfall: Gib keine Navigationshinweise und erwarte keinen Systemkontext.

Regeln:
- Antworte auf Deutsch in klar gesprochener Sprache, ohne jede Formatierung: keine Sternchen, keine Aufzählungszeichen, keine Überschriften, kein Markdown. Nur kurze, fließende Sätze.
- Enthält das Bild Text, lies ihn vollständig und wörtlich vor — bei einem Brief, Etikett, Beipackzettel oder einer Rechnung ist das meist das Wichtigste. Beginne mit dem Wesentlichen wie Absender, Betreff, Betrag, Datum oder Warnhinweisen.
- Enthält das Bild sowohl Text als auch Bilder, Fotos, Logos oder Grafiken, dann lies den Text vor UND beschreibe an der passenden Stelle kurz, was die Bilder darin zeigen, damit beides ankommt.
- Ist es ein reines Foto oder eine Szene ohne nennenswerten Text, beschreibe knapp und konkret, was zu sehen ist.
- Erfinde nichts, was nicht im Bild steht. Ist etwas unscharf oder nicht lesbar, sag das ehrlich.
"""
...
"""
    clipboard_png() -> Union{Vector{UInt8}, Nothing}

Holt das aktuelle Bild aus der Windows-Zwischenablage ueber PowerShell
(Clipboard.GetImage) und liefert PNG-Bytes, oder nothing, wenn dort kein Bild liegt.
"""
function clipboard_png()::Union{Vector{UInt8},Nothing}
    tmp = joinpath(tempdir(), "saywhatson_clip.png")
    isfile(tmp) && rm(tmp; force=true)
    ps = """
    Add-Type -AssemblyName System.Windows.Forms, System.Drawing
    \$img = [System.Windows.Forms.Clipboard]::GetImage()
    if (\$img -eq \$null) { Write-Output 'LEER' } else {
    ...
    out = try
        ...
        @error "Zwischenablage lesen fehlgeschlagen" exception = e
        return nothing
    end
    (occursin("OK", out) && isfile(tmp)) || return nothing
    return read(tmp)
...
"""
    beschreibe_bild(b::GeminiBackend, png, kontext="", prompt=PROMPT) -> String

Sendet Prompt + optionalen Betriebssystem-Kontext + Bild an Gemini (Cloud) und
liefert den deutschen Text. Keine automatische Wiederholung.
"""
function beschreibe_bild(b::GeminiBackend, png::Vector{UInt8},
                         kontext::AbstractString="", prompt::AbstractString=PROMPT)::String
...
"""
    beschreibe_bild(b::OllamaBackend, png, kontext="", prompt=PROMPT) -> String

Wie die Gemini-Variante, aber gegen eine lokale Ollama-Instanz (kein Datenabfluss).
"""
function beschreibe_bild(b::OllamaBackend, png::Vector{UInt8},
                         kontext::AbstractString="", prompt::AbstractString=PROMPT)::String
...
"""
    fehlermeldung(e) -> String

Wandelt eine gefangene Ausnahme in eine vorlesbare, freundliche Fehlermeldung.
"""
function fehlermeldung(e)::String
    if e isa HTTP.Exceptions.StatusError
        if e.status in (429, 500, 502, 503, 504)
            return "Der Bilddienst ist gerade überlastet. Bitte versuche es in ein paar Sekunden noch einmal."
        elseif e.status in (400, 401, 403)
            return "Zugriff verweigert. Das liegt wahrscheinlich am API-Schlüssel oder am Kontingent; ein erneuter Versuch hilft hier nicht."
        else
            return "Der Bilddienst meldet einen Fehler mit der Nummer $(e.status). Bitte versuche es später noch einmal."
        end
    end
    return "Es gab einen unerwarteten Fehler. Bitte versuche es noch einmal."
end
...
function verarbeite(backend::ImageBackend)::Nothing
    ...
    try
        ...
        beep_error()
        ausgeben(fehlermeldung(e))
        e isa HTTP.Exceptions.StatusError || @error "Technischer Fehler" exception = (e, catch_backtrace())
    end
    return nothing
end

"""
    verarbeite_clipboard(backend::ImageBackend) -> Nothing

beep_start, Bild aus der Zwischenablage holen, mit dem Bild-Prompt (Foto/Dokument)
beschreiben lassen, merken, beep_ready, sprechen. Liegt kein Bild vor, wird das gesagt.
"""
function verarbeite_clipboard(backend::ImageBackend)::Nothing
    beep_start()
    png = clipboard_png()
    if png === nothing
        beep_error()
        ausgeben("In der Zwischenablage ist kein Bild. Kopiere zuerst eines, zum Beispiel mit Alt und Druck für das aktive Fenster, mit dem Snipping Tool oder über die Scan-Software.")
        return nothing
    end
    try
        text = beschreibe_bild(backend, png, "", PROMPT_BILD)
        LETZTE_BESCHREIBUNG[] = text
        beep_ready()
        ausgeben(text)
    catch e
...
Registriert Strg+Druck (Bildschirm), Strg+Umschalt+Druck (Zwischenablage) und
Umschalt+Druck (Editor), laeuft bis Strg+C und gibt alle Hotkeys beim Beenden frei.
"""
function main(; backend::ImageBackend = GeminiBackend())::Nothing
    ...
    ok3 = ccall((:RegisterHotKey, "user32"), Cint, (Ptr{Cvoid}, Cint, Cuint, Cuint),
                C_NULL, HOTKEY_CLIPBOARD, MOD_CONTROL | MOD_SHIFT | MOD_NOREPEAT, VK_SNAPSHOT)
    ...
    ok3 == 0 && @warn "RegisterHotKey (Strg+Umschalt+Druck) fehlgeschlagen — Zwischenablage-Taste nicht verfuegbar."
    try
        ...
        println("  Strg+Umschalt+Druck = Bild aus der Zwischenablage beschreiben")
        ...
        hotkey_loop() do id
            ...
            elseif id == Int(HOTKEY_CLIPBOARD)
                verarbeite_clipboard(backend)
            end
        end
    finally
        ...
        ccall((:UnregisterHotKey, "user32"), Cint, (Ptr{Cvoid}, Cint), C_NULL, HOTKEY_CLIPBOARD)
    end
    return nothing
end
...
end# src/saywhatson.jl — SayWhatsOn, Meilenstein 3 (Hotkey + Aufnahme + Sprache + Editor + Systemkontext + Fensterliste)
#
# 📍 ORT IN DER PROJEKTARCHITEKTUR:
